...
export const SHAPES = {
  ...
  speaker(p, item, origin, t) {
    ...
    k.box(0.42, face - 0.02, k.W - 0.84, 0.05, 0.29, 0.14, '#241d33')
  ...
  fridge(p, item, origin) {
    ...
    k.box(0.14, k.D - 0.13, k.W - 0.28, 0.05, 0.1, 1.02, porta)   // freezer embaixo
    k.box(0.14, k.D - 0.13, k.W - 0.28, 0.05, 1.2, 0.68, porta)   // geladeira em cima

    // puxadores verticais, um por porta, na beirada esquerda de quem olha
    k.box(k.W - 0.26, k.D - 0.17, 0.05, 0.06, 0.45, 0.45, '#9a8f86')
    k.box(k.W - 0.26, k.D - 0.17, 0.05, 0.06, 1.42, 0.34, '#9a8f86')

    // ímã de geladeira e um bilhetinho — o detalhe que faz parecer casa de gente
    k.box(0.26, k.D - 0.16, 0.14, 0.03, 1.62, 0.12, '#f2b33d')
    k.box(0.24, k.D - 0.16, 0.2, 0.03, 1.3, 0.22, '#fdf4ea')
  },
  ...
  candles(p, item, origin, t) {
    ...
    velas.forEach(([lx, ly, h], i) => {
      ...
      k.box(lx + 0.05, ly + 0.05, 0.04, 0.04, 0.16 + h, alto * 0.7, '#fff0b8')
    })
  },
  ...
  frame(p, item, origin) {
    ...
    k.box(0.12, 0.06, k.W - 0.24, 0.08, 1.05, 0.78, moldura)
    k.box(0.2, 0.1, k.W - 0.4, 0.04, 1.14, 0.6, '#f3ead9')  // paspatur
    k.box(0.26, 0.12, k.W - 0.52, 0.03, 1.22, 0.44, '#a8cde0') // a foto
    // um horizonte simples dentro da foto, pra não ser um retângulo azul
    k.box(0.26, 0.12, k.W - 0.52, 0.03, 1.22, 0.16, '#7fae7d')
  },

  frame_couple(p, item, origin) {
    const k = tools(p, item, origin)
    // Mesma correção do `frame`: encostado na parede de trás.
    const moldura = '#8a6a4a'
    k.box(0.1, 0.06, k.W - 0.2, 0.08, 0.98, 0.92, moldura)
    k.box(0.18, 0.1, k.W - 0.36, 0.04, 1.08, 0.72, '#f3ead9')
    k.box(0.24, 0.12, k.W - 0.48, 0.03, 1.16, 0.56, '#cfe4d8')
    // os dois rostinhos, agora em BLOCO e não em pixel de tela.
    //
    // Antes eram `p.rect` em coordenada de tela: não giravam com o móvel, então
    // ao virar o quadro os rostos ficavam para o lado errado, soltos do vidro.
    k.box(0.3, 0.11, 0.16, 0.02, 1.3, 0.2, '#ffd9c0')
    k.box(k.W - 0.46, 0.11, 0.16, 0.02, 1.3, 0.2, '#f6c9b0')
    k.box(0.33, 0.1, 0.04, 0.02, 1.4, 0.04, '#33203a')
    k.box(0.41, 0.1, 0.04, 0.02, 1.4, 0.04, '#33203a')
    k.box(k.W - 0.43, 0.1, 0.04, 0.02, 1.4, 0.04, '#33203a')
    k.box(k.W - 0.35, 0.1, 0.04, 0.02, 1.4, 0.04, '#33203a')
    k.box(k.W / 2 - 0.05, 0.1, 0.1, 0.02, 1.54, 0.08, '#e8879b') // coraçãozinho
  },
  ...
  stove(p, item, origin, t) {
    ...
    const face = k.D - 0.09
    k.box(0.12, face, k.W - 0.24, 0.04, 0.08, 0.44, '#c6c0b7') // porta do forno
    k.box(0.2, face - 0.01, k.W - 0.4, 0.04, 0.16, 0.28, '#4a4550') // visor
    k.box(0.16, face - 0.02, k.W - 0.32, 0.05, 0.54, 0.05, '#9a938a') // puxador
    for (let i = 0; i < 3; i++) {
      k.box(0.2 + i * 0.22, face - 0.01, 0.09, 0.04, 0.6, 0.07, '#8f8a84') // botões
    }
  },
  ...
  petbowl(p, item, origin) {
    ...
    k.box(0.19, 0.31, 0.24, 0.24, 0.13, 0.05, '#8fd0ef')
    k.box(k.W - 0.48, 0.26, 0.34, 0.34, 0.03, 0.16, '#c98a4b') // tigela da ração
    k.box(k.W - 0.43, 0.31, 0.24, 0.24, 0.13, 0.06, '#8a5f3c')
  ...
  grill(p, item, origin, t) {
    ...
    k.box(0.14, 0.2, k.W - 0.28, k.D - 0.4, 0.42, 0.3, '#55505a') // cuba
    k.box(0.2, 0.26, k.W - 0.4, k.D - 0.52, 0.5, 0.06, '#7a2f22') // brasa
    for (let i = 0; i < 3; i++) {
      k.box(0.26 + i * 0.2, 0.32, 0.12, k.D - 0.64, 0.56, 0.03, '#e8724a') // carvão aceso
    }
    // a grelha: barras separadas, que é o que faz parecer grelha
    for (let i = 0; i < 5; i++) {
      k.box(0.18 + i * 0.15, 0.22, 0.07, k.D - 0.44, 0.66, 0.04, '#2b282f')
    }
